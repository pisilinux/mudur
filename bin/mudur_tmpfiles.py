    ...
    if sys.argv[1:] and not boot:
        for arg in sys.argv[1:]:
            ...
            if not tail.endswith(".conf"):
                errors.append("%s is not .conf file" % tail)
            elif not head:
                errors.append("Full path is needed for %s args." % sys.argv[0])
            elif not os.path.isdir(head):
                errors.append("Path %s not exists." % head)
            elif not os.path.isfile(arg):
                errors.append("File %s not exists." % arg)
    ...
    for d in DEFAULT_CONFIG_DIRS_EO:
        ...
        for f in fs:
            ...
            for line in [l for l in conf.split("\n") if l and not (l.startswith("#") or l.isspace())]:
                cerr = len(errors)
                fields = line.split()
                if len(fields) < 3: errors.append(
                    "%s is invalid .conf file. Not enough args in line: %s" % (os.path.join(d, f), line))
                if len(fields) < 7:
                    fields.extend(["", ] * (7 - len(fields)))
                elif len(fields) > 7:
                    fields = fields[0:6] + [re.sub(".*?(%s)\s*$" % "\s+".join(fields[6:]), "\\1", line)]
                if fields[0] == "c" and not re.search("\d+:\d+", fields[6]): errors.append(
                    "%s - wrong argument for type 'c' in file: %s" % (fields[6], os.path.join(d, f)))
                ...
                if not fields[0] in ["c", "d", "D", "f", "F", "L", "w"]:
                    ...
                    errors.append("Cannot write to file. %s is directory." % fields[1])
                else:
                    if not fields[2]:
